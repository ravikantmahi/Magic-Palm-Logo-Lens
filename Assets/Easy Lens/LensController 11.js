...
//@input Component.ScriptComponent blackGlasses
...
var sparkles = [];
var rngSeed = 0;
var activeHand = null; // 'left' | 'right' | null
var leftPalmOpen = false;
var rightPalmOpen = false;
...
function hideAll() {
    // Hard hide everything and clear state, used on fist/hand lost
    if (script.nielitLogoSticker) {
        script.nielitLogoSticker.enabled = false;
    }
    if (glowCanvas) {
...
script.createEvent("OnStartEvent").bind(function() {
    ...
    // Ensure glasses are enabled independently of palm logic and always on
    if (script.blackGlasses) {
        script.blackGlasses.enabled = true; // keep eyewear always visible
    }
...
script.handTracking.onLeftPalmOpen.add(function() {
    leftPalmOpen = true;
    // Choose active hand as the most recently opened if none or right isn't active
    if (activeHand === null || activeHand === 'right') {
        activeHand = 'left';
    }
    if (activeHand === 'left' && !hasPlayedEntry) {
        startEntry(new Date().getTime() / 1000.0);
    }
});
script.handTracking.onRightPalmOpen.add(function() {
    rightPalmOpen = true;
    if (activeHand === null || activeHand === 'left') {
        activeHand = 'right';
    }
    if (activeHand === 'right' && !hasPlayedEntry) {
        startEntry(new Date().getTime() / 1000.0);
    }
});
...
function handleTrackingData(data) {
    ...
    // Clamp to safe on-screen range (pixels)
    px.x = MathUtils.clamp(px.x, 0, screenSize.x);
    px.y = MathUtils.clamp(px.y, 0, screenSize.y);
    ...
    if (script.nielitLogoSticker && hasPlayedEntry) {
        ...
        // Clamp normalized back to 0..1 after conversion
        var nx = MathUtils.clamp(newPos.x / screenSize.x, 0, 1);
        var ny = MathUtils.clamp(newPos.y / screenSize.y, 0, 1);
        script.nielitLogoSticker.position = new vec2(nx, ny);
...
script.createEvent("UpdateEvent").bind(function() {
    ...
    // Safety: if no active open palm, enforce hidden each frame
    var hasOpen = (activeHand === 'left' && leftPalmOpen) || (activeHand === 'right' && rightPalmOpen);
    if (!hasOpen) {
        hideAll();
    }
